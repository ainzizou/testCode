...
if [ $? == 0 -a -n "${asg}" ]; then
    ...
    echo "Attempting to move instance out of Standby"
    autoscaling_exit_standby $INSTANCE_ID "${asg}"
    if [ $? != 0 ]; then
        error_exit "Failed to move instance out of standby"
    else
        echo "Instance is no longer in Standby"
        finish_echo
        exit 0
    fi
fi

echo "Instance is not part of an ASG, continuing with ELB"
...
if [ -z "$ELB_LIST" ]; then
    ...
    if [ "$(get_flag "dereg")" = "true" ]; then
        echo "Finding all the ELBs that this instance was previously registered to"
        ...
        echo "Assuming this is the first deployment and ELB_LIST=_all_ so finishing successfully without registering."
        finish_echo
        exit 0
    fi
    ...
    if [ "$(get_flag "dereg")" = "true" ]; then
        ...
        if ! ELB_LIST=$(get_flag "ELBs"); then
            ...
            echo "Couldn't find any, but ELB_LIST=_any_ so finishing successfully without registering."
            remove_flagfile
            finish_echo
            exit 0
        fi
    else
        echo "Assuming this is the first deployment and ELB_LIST=_any_ so finishing successfully without registering."
        finish_echo
        exit 0
    fi
fi
...
for elb in $ELB_LIST; do
    ...
    echo "Registering $INSTANCE_ID to $elb"
...
echo "Waiting for instance to register to its load balancers"
for elb in $ELB_LIST; do
